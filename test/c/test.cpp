...
BOOST_AUTO_TEST_CASE(light_and_full_client_checks) {
	...
	BOOST_ASSERT(full);
	{
		const std::string
				...
				actual = bytesToHexString((uint8_t const *) cache->mem, cache_size);
	...
	{
		node node;
		ethash_calculate_dag_item(&node, 0, cache);
		const std::string
	...
	}
	{
		for (int i = 0; i < full_size / sizeof(node); ++i) {
			for (uint32_t j = 0; j < 32; ++j) {
				node expected_node;
				ethash_calculate_dag_item(&expected_node, j, cache);
				const std::string
	...
	}
	{
		...
		BOOST_REQUIRE(ethash_light_compute(&light_out, light, full_size, &hash, 5));
		light_result_string = blockhashToHexString(&light_out.result);
		BOOST_REQUIRE_MESSAGE(light_result_string == full_result_string,
...
BOOST_AUTO_TEST_CASE(full_client_callback) {
	...
	ethash_cache *cache = ethash_cache_new(cache_size, &seed);
	ethash_full_t full = ethash_full_new(
		"./test_ethash_directory/",
		&seed,
		full_size,
...
BOOST_AUTO_TEST_CASE(light_and_full_client_checks_old) {
	...
	ethash_compute_full_data(full_mem, params.full_size, &cache);
